...
PLL_Init
    ...
    BIC R0, R0, #SYSCTL_RSCLKCFG_USEPLL        ; R0 = R0&~SYSCTL_RSCLKCFG_USEPLL (limpar o bit USEPLL bit para nao clockar pelo PLL)
    STR R0, [R1]                               ; [R1] = R0
...
PLL_Init_step3loop
    ...
    STR R0, [R1]                               ; [R1] = R0
    ; 5) Se a aplicacao tambem necessita que o MOSC seja a fonte de clock para deep-sleep
    ;    entao programar o campo DSOSCSRC no registrador SYSCTL_DSCLKCFG_R para 0x3.
    ...
    ;    Q, N, MINT, e MFRAC para configurar as configuracoes desejadas da frequencia de VCO.
    ;    ************
    ;    fVC0 = (fXTAL/(Q + 1)/(N + 1))*(MINT + (MFRAC/1,024))
    ;    fVCO = 480,000,000 Hz (arbitrary, but presumably as small as needed)
	;    Para uma frequencia que nao seja um divisor inteiro de 480 MHz, mudar esta secao
FXTAL  EQU 25000000                 ; fixa, o cristal esta soldado no Launchpad
Q      EQU        0
N      EQU        4                 ; escolhido para ser a frequencia de referencia entre 4 e 30 MHz
...
SYSCLK EQU (FXTAL/(Q+1)/(N+1))*(MINT+MFRAC/1024)/(PSYSDIV+1)
    ...
    LDR R3, =SYSCTL_PLLFREQ0_MFRAC_M           ; R3 = SYSCTL_PLLFREQ0_MFRAC_M (mascar)
    ...
    LDR R3, =SYSCTL_PLLFREQ0_MINT_M            ; R3 = SYSCTL_PLLFREQ0_MINT_M (mascara)
    ...
    STR R0, [R1]                               ; [R1] = R0 (MFRAC e MINT alteraram mas nao estao fixados ainda)
    ...
    STR R0, [R1]                               ; [R1] = R0 (Q e N alteraram mas nao estao fixados ainda)
    LDR R1, =SYSCTL_PLLFREQ0_R                 ; R1 = SYSCTL_PLLFREQ0_R (ponteiro)
    LDR R0, [R1]                               ; R0 = [R1] (valor)
    ...
    ;    ************
    ;    Configurar os parametros de tempo para as memorias Flash e EEPROM que 
	;    dependem da frequencia do clock do sistema. Ver a Tabela 5-12 do datasheet.
    LDR R1, =SYSCTL_MEMTIM0_R                  ; R1 = SYSCTL_MEMTIM0_R (ponteiro)
    LDR R0, [R1]                               ; R0 = [R1] (valor)
    LDR R3, =0x03EF03EF                        ; R3 = 0x03EF03EF (mascara)
...
PLL_Init_step7veryfast                         ; 80 MHz < SysClk <= 100 MHz: "very fast"
    ...
    CMP R2, R3                                 ; Eh R2 (SysClk) <= R3 (80,000,000 Hz)?
...
PLL_Init_step7fast                             ; 60 MHz < SysClk <= 80 MHz: "fast"
    ...
    CMP R2, R3                                 ; Eh R2 (SysClk) <= R3 (60,000,000 Hz)?
...
PLL_Init_step7medium                           ; 40 MHz < SysClk <= 60 MHz: "medium"
    ...
    CMP R2, R3                                 ; Eh R2 (SysClk) <= R3 (40,000,000 Hz)?
...
PLL_Init_step7slow                             ; 16 MHz < SysClk <= 40 MHz: "slow"
    ...
    CMP R2, R3                                 ; Eh R2 (SysClk) <= R3 (16,000,000 Hz)?
...
PLL_Init_step7done
    STR R0, [R1]                               ; [R1] = R0 (SYSCTL_MEMTIM0_R alterado mas nao fixado ainda)
    ; 8) Espera pelo registrador SYSCTL_PLLSTAT_R indicar que o PLL atingiu travamento
    ;    no novo ponto de operacao (ou que um periodo de timeout passou e o travamento
    ;    falhou, que no caso uma condicao de erro existe e esta sequencia eh abandonada
...
PLL_Init_step8done
    ...
    ORR R0, R0, #SYSCTL_RSCLKCFG_MEMTIMU       ; R0 = R0|SYSCTL_RSCLKCFG_MEMTIMU (setar o bit MEMTIMU para atualizar os parametros de temporizacao de memoria)
...
SysTick_Init
	...
	LDR R0, =0x00FFFFFF; 			; valor maximo de recarga 2^24 ticks
...
; Entrada: R0 -> parametro de delay em unidades do clock do core (12.5ns)
; Saida: Nenhum
; Modifica: R0
SysTick_Wait
	...
	SUB R0, #1                          
	STR R0, [R1] 						; delay-1, numero de contagens para esperar
	LDR R1, =NVIC_ST_CTRL_R 			; R1 = &NVIC_ST_CTRL_R
SysTick_Wait_loop
	LDR R3, [R1] 						; R3 = &NVIC_ST_CTRL_R (ponteiro)
	ANDS R3, R3, #0x00010000 			; O bit COUNT esta setado? (Bit 16)
	BEQ SysTick_Wait_loop               ; Se sim permanece no loop
	POP {R1, R3}						; Restaura
	BX LR                               ; Se nao, retorna

;------------SysTick_Wait1ms------------
; tempo de atraso usando processador ocupado. Assume um clock de 80 MHz
; Entrada: R0 --> Numero de vezes para contar 1ms.
; Saida: Nao tem
; Modifica: R0
DELAY1MS EQU 80000 ; numero de ciclos de clock para contar 1ms (assumindo 80 MHz)
...
SysTick_Wait1ms_loop					
	LDR R0, =DELAY1MS 					; R0 = DELAY1MS (numero de ticks para contar 1ms)
...
;------------SysTick_Wait1us------------
; tempo de atraso usando processador ocupado. Assume um clock de 80 MHz
; Entrada: R0 --> Numero de vezes para contar 1us.
; Saida: Nao tem
; Modifica: R0
DELAY1US EQU 80    ; numero de ciclos de clock para contar 1ms (assumindo 80 MHz)
	               ; 80000 x 12,5 ns = 1 ms
...
SysTick_Wait1us_loop					
	LDR R0, =DELAY1US 					; R0 = DELAY1MS (numero de ticks para contar 1ms)
	BL SysTick_Wait 					; chama a rotina para esperar por 1ms
	SUBS R4, R4, #1 					; R4 = R4 - 1; numEsperasRestantes--
